...
func(a *Auditor)Write(entry AuditEntry)error{a.mu.Lock();defer a.mu.Unlock();if err:=os.MkdirAll(filepath.Dir(a.path),0755);err!=nil{return err};if info,err:=os.Stat(a.path);err==nil&&info.Size()>=maxAuditLogBytes{_ = os.Remove(a.path+".1");if err:=os.Rename(a.path,a.path+".1");err!=nil{return err}};file,err:=os.OpenFile(a.path,os.O_APPEND|os.O_CREATE|os.O_WRONLY,0644);if err!=nil{return err};defer file.Close();return json.NewEncoder(file).Encode(entry)}
